#!/usr/bin/env python
"""5-fold cross-validation for 9 rate prediction models.
...
Models (ordered by complexity):
  ...
  5. DiffAttention_reg       - Token diff + TransformerEncoder
  6. EditRNA_rate            - Full EditRNA-A3A (CachedRNAEncoder, rate_weight=10)
  7. GatedFusion_small       - 3-way gated fusion (primary + delta + hand)
  8. EditRNA+Feat_heavyreg   - EditRNA + hand features via GatedModalityFusion
  9. 4Way_heavyreg           - 4-way gated fusion with GNN

Usage:
    python experiments/apobec3a/exp_rate_5fold.py
"""
...
def run_experiment():
    ...
    logger.info("=" * 70)
    logger.info("5-FOLD CROSS-VALIDATION RATE PREDICTION EXPERIMENT")
    logger.info("=" * 70)
    ...
    for fold_idx, (remain_idx, test_idx) in enumerate(kf.split(all_site_ids)):
        ...
        # 3. GB_HandFeatures
        # ==============================================================
        logger.info("  [3/9] GB_HandFeatures")
        try:
        ...
        # 4. GB_AllFeatures
        # ==============================================================
        logger.info("  [4/9] GB_AllFeatures")
        try:
            from xgboost import XGBRegressor
        ...
        # ==============================================================
        # 5. DiffAttention_reg (needs token-level embeddings)
        # ==============================================================
        logger.info("  [5/9] DiffAttention_reg")
        torch.manual_seed(fold_seed)
        ...
        diff_metrics = train_nn_model(
            diff_model, token_loaders, "DiffAttention_reg",
            epochs=100, lr=1e-3, weight_decay=1e-2, patience=15)
        model_fold_results["DiffAttention_reg"].append(diff_metrics)
        del diff_model

        # ==============================================================
        # 6. EditRNA_rate (needs token-level embeddings via CachedRNAEncoder)
        # ==============================================================
        logger.info("  [6/9] EditRNA_rate")
        try:
            torch.manual_seed(fold_seed)
            from models.editrna_a3a import EditRNA_A3A, EditRNAConfig
        ...
        # ==============================================================
        # 7. GatedFusion_small (needs pooled + hand features)
        # ==============================================================
        logger.info("  [7/9] GatedFusion_small")
        torch.manual_seed(fold_seed)
        mm_loaders = make_mm_loaders(
            train_sids, val_sids, test_sids,
            train_targets, val_targets, test_targets,
            train_hand, val_hand, test_hand, data)
        gated_model = GatedFusionRateModel(
            d_model=640, d_hand=d_hand, d_proj=64, dropout=0.3)
        gated_metrics = train_nn_model(
            gated_model, mm_loaders, "GatedFusion_small",
            epochs=200, lr=1e-3, weight_decay=5e-3, patience=25)
        model_fold_results["GatedFusion_small"].append(gated_metrics)
        del gated_model

        # ==============================================================
        # 8. EditRNA+Feat_heavyreg (needs pooled + hand features)
        # ==============================================================
        logger.info("  [8/9] EditRNA+Feat_heavyreg")
        try:
            torch.manual_seed(fold_seed)
            from models.encoders import CachedRNAEncoder

            cached_encoder_pooled = CachedRNAEncoder(
                tokens_cache=None,
                pooled_cache=data["pooled_orig"],
            ...
            aug_model = EditRNARateAugmented(
                d_hand=d_hand, d_edit=128, d_fused=256,
                cached_encoder=cached_encoder_pooled, dropout=0.5)
            aug_metrics = train_editrna_augmented(
                aug_model, mm_loaders,
                epochs=200, lr=5e-4, weight_decay=5e-2, patience=30)
            model_fold_results["EditRNA+Feat_heavyreg"].append(aug_metrics)
            del aug_model, cached_encoder_pooled
        except Exception as e:
            logger.error("EditRNA+Feat_heavyreg FAILED fold %d: %s", fold_idx + 1, e)
            model_fold_results["EditRNA+Feat_heavyreg"].append(
                {"spearman": float("nan"), "pearson": float("nan"),
                 "mse": float("nan"), "r2": float("nan")})
        ...
        # ==============================================================
        # 9. 4Way_heavyreg (needs pooled + hand + GNN embeddings)
        # ==============================================================
        logger.info("  [9/9] 4Way_heavyreg")
    ...
    def _count_params(model_name):
        ...
        param_counts = {
            ...
            "GB_HandFeatures": 0,
            "GB_AllFeatures": 0,
            "DiffAttention_reg": sum(p.numel() for p in DiffAttentionRegressor().parameters()),
            "GatedFusion_small": sum(
                p.numel() for p in GatedFusionRateModel(d_hand=d_hand, d_proj=64).parameters()),
            "4Way_heavyreg": sum(
    ...
    for name in MODEL_NAMES:
        ...
        elif name == "EditRNA+Feat_heavyreg":
            n_params = -1
