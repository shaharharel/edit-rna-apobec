...
# Run: bash scripts/apobec3a/rebuild_pipeline_hg38.sh
set -e
...
$PYTHON scripts/apobec3a/extract_sequences_and_structures.py --genome "$GENOME"
...
# Step 3b: Filter to A3A-only sites
echo ""
echo "=== Step 3b: Filter to APOBEC3A-only sites ==="
...
$PYTHON scripts/apobec3a/generate_hardneg_pipeline.py --neg-ratio 5 --candidates-ratio 20 --workers 4
...
$PYTHON experiments/apobec3a/exp0_tabular_baseline.py 2>&1 | tail -5

echo "  6b: Dataset matrix..."
$PYTHON experiments/apobec3a/exp_dataset_matrix.py 2>&1 | tail -5

echo "  6c: Rate prediction..."
$PYTHON experiments/apobec3a/exp_rate_prediction.py 2>&1 | tail -5

echo "  6d: Rate deep dive..."
$PYTHON experiments/apobec3a/exp_rate_deep_dive.py 2>&1 | tail -5

echo "  6e: TC-motif reanalysis..."
$PYTHON experiments/apobec3a/exp_tc_motif_reanalysis.py 2>&1 | tail -5

echo "  6f: ClinVar prediction..."
$PYTHON experiments/apobec3a/exp_clinvar_prediction.py 2>&1 | tail -5

echo "  6g: Hard negative baselines..."
$PYTHON experiments/apobec3a/exp_hardneg_baselines.py 2>&1 | tail -5
