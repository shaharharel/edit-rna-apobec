#!/usr/bin/env python3
"""TCGA somatic mutation enrichment with FULL 40-dim GB model (ViennaRNA structure features).

Re-runs the TCGA analysis from tcga_gnomad_editability.py but with the full hand-crafted
feature set (motif 24 + structure delta 7 + loop geometry 9) instead of MotifOnly (24-dim).

Uses heavy parallelization (all available CPUs) for ViennaRNA folding.

Usage:
    conda run -n quris python scripts/multi_enzyme/tcga_full_model_enrichment.py
"""
...
VIENNA_CACHE = OUTPUT_DIR / "vienna_cache_tcga.npz"

N_CONTROLS = 5  # matched controls per mutation
N_WORKERS = max(1, mp.cpu_count() - 2)  # leave 2 CPUs free
SEED = 42
...
def fold_single_sequence(seq_201nt):
    ...
    try:
        ...
        # Extract pairing probabilities at center ±10 (compact storage)
...
def parallel_fold(sequences, n_workers=N_WORKERS, chunk_size=500):
    ...
    Args:
        sequences: list of 201-nt strings
        n_workers: number of parallel workers
        chunk_size: sequences per work unit

    Returns:
        list of fold results (dict or None), same length as input
    """
    n = len(sequences)
...
def build_40dim_features(sequences, fold_results):
    ...
    Args:
        sequences: list of 201-nt strings
        fold_results: list of RAW fold result dicts (from parallel_fold)

    Returns:
        np.ndarray of shape [N, 40], with NaN-to-zero for failed folds
    """
    n = len(sequences)
    features = np.zeros((n, 40), dtype=np.float32)

    for i, (seq, fold_raw) in enumerate(zip(sequences, fold_results)):
        # Motif features (24-dim) — always available
        if seq is not None and len(seq) == 201:
            features[i, :24] = extract_motif_from_seq(seq)

        # Structure features (16-dim) — derive from raw fold
...
def download_maf(cancer_type):
    ...
    logger.info(f"Downloading {cancer_type} MAF...")
...
def parse_ct_mutations(maf_path):
    ...
    # Deduplicate by position (keep unique genomic positions)
...
def get_matched_controls(mutations_df, genome, exons_by_gene, n_controls=N_CONTROLS):
    ...
    for _, row in mutations_df.iterrows():
        ...
        # Collect all C positions in gene exons
        c_positions = []
        for chrom, start, end, strand, _ in gene_exons:
            ...
            try:
                ...
                for i, base in enumerate(exon_seq):
                    pos = start + i
                    if pos == row["pos"]:
                        continue  # skip the mutation itself
                    if base == "C":
...
def main():
    ...
    logger.info("=" * 60)
    logger.info("Step 1: Parse exonic regions")
    exons = []
    ...
    all_results = {}
    for cancer in ["blca", "brca", "cesc", "lusc", "skcm"]:
        logger.info("=" * 60)
        logger.info(f"Step 4: Processing {cancer.upper()}")
        ...
        # Save raw scores to CSV for post-hoc analysis
        ...
        # Compute enrichment at many thresholds
        ...
        # Free memory (fold_results already cached to disk)
        del features_40d, scores, fold_results
        gc.collect()

    # Save results
    with open(OUTPUT_DIR / "tcga_full_model_results.json", "w") as f:
        json.dump(all_results, f, indent=2, default=str)

    # Generate figure
    logger.info("Generating figures...")
    cancers = [c for c in ["blca", "brca", "cesc", "lusc", "skcm"] if c in all_results]
    ...
    ax.set_title("Full 40-dim GB Model")
    ax.legend()

    # Panel 2: OR at multiple thresholds
    ax = axes[1]
    for cancer in cancers:
        color = "#dc2626" if cancer != "skcm" else "#94a3b8"
        thresholds = [0.3, 0.4, 0.5, 0.6, 0.7, 0.8, 0.9, 0.95]
        ors = [all_results[cancer]["enrichment"].get(str(t), {}).get("OR", float("nan"))
               for t in thresholds]
        ax.plot(thresholds, ors, marker="o", label=cancer.upper(), color=color,
                linewidth=2 if cancer != "skcm" else 1,
                linestyle="-" if cancer != "skcm" else "--")
    ...
    # Panel 3: OR at t=0.5 comparison
    ax = axes[2]
    ors_05 = [all_results[c]["enrichment"]["0.5"]["OR"] for c in cancers]
    colors = ["#dc2626" if c != "skcm" else "#94a3b8" for c in cancers]
    bars = ax.bar(x, ors_05, color=colors, alpha=0.8)
    ax.axhline(1.0, color="gray", linestyle=":", alpha=0.5)
    ax.set_xticks(x)
    ax.set_xticklabels([c.upper() for c in cancers])
    ...
    plt.tight_layout()
    plt.savefig(OUTPUT_DIR / "tcga_full_model_enrichment.png", dpi=150, bbox_inches="tight")
    plt.close()

    elapsed = time.time() - t_start
    logger.info(f"\n{'='*60}")
    logger.info(f"COMPLETE ({elapsed/60:.0f} min)")
    logger.info(f"Results: {OUTPUT_DIR / 'tcga_full_model_results.json'}")
    logger.info(f"Figure: {OUTPUT_DIR / 'tcga_full_model_enrichment.png'}")
